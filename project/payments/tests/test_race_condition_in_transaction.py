...
def _register_sender(live_server):
    ...
    register = requests.post(
        f"{live_server.url}{reverse('api-registration')}", data=sender_data
    )
    assert register.status_code == 200, register.text
    login = requests.post(f"{live_server.url}{reverse('api-login')}", data=sender_data)
...
def test_transfer_to_another_user_with_lag_success(live_server, event_loop, settings):
    ...
    receiver_wallet = WalletFactory(balance=0, user__email=RECEIVER_EMAIL)
    ...
    async def create_transactions():
        async with aiohttp.ClientSession(
            ...
            await asyncio.gather(
                create_transaction_success(client), create_transaction_fail(client)
            )

    event_loop.run_until_complete(create_transactions())
