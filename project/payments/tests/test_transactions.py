...
def test_make_deposits_for_another(ac):
    ...
    receiver = WalletFactory(balance=0,
...
def test_transfer_to_another_user_success(ac):
    ...
    response = ac.post(reverse('transaction-transfer-to-another-user'), data=data, format='json')
...
def test_invoice_create(ac):
    ...
    # -------------------------------------------------------------
    assert response.status_code == 200
    assert Transaction.objects.filter(receiver__user__email=RECEIVER_EMAIL,
...
def test_invoice_pay_success(ac):
    ...
    # -------------------------------------------------------------
    ac.force_authenticate(user=sender.user)
    response = ac.post(reverse('transaction-pay-invoice'), data={'uuid': invoice_uuid}, format='json')
    ...
    assert Transaction.objects.filter(uuid=invoice_uuid,
                                      is_done=True).exists()
